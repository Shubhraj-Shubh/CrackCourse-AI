              ...
              {chapter?.topics.map((topic, index) => (
                <div className='flex flex-col items-center' key={index}>
                  ...
                  <div className='flex items-center gap-5'>
                    ...
                    <h2 className='text-center rounded-full bg-gray-300 px-6 text-gray-500 py-4'>{index+1}</h2>
                    <span className={`${index % 2 != 0 && 'text-transparent'} max-w-xs`}>{topic}</span>
                  </div>
                ...
                {index == chapter?.topics?.length - 1 && <div className='h-10 bg-grey-300 w-4'>  </div>}
                </div>
              ))}
              </div> 
            </div>
